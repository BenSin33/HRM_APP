...
public class EmployeeManagementPanel extends JPanel {
    ...
    private JPanel createHeader() {
        ...
        exportBtn.setPreferredSize(new java.awt.Dimension(100, 32));

        JButton importBtn = new JButton("Nhập Excel");
        importBtn.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        importBtn.setForeground(Color.WHITE);
        importBtn.setBackground(new Color(33, 150, 243));
        importBtn.setFocusPainted(false);
        importBtn.setPreferredSize(new java.awt.Dimension(100, 32));
        ...
        if (currentUser == null || !permissionService.canExport(currentUser, "CN02_EMPLOYEE")) {
            ...
            exportBtn.setToolTipText("Bạn không có quyền xuất Excel");
        }
        if (currentUser == null || !permissionService.canAdd(currentUser, "CN02_EMPLOYEE")) {
            importBtn.setEnabled(false);
            importBtn.setToolTipText("Bạn không có quyền nhập Excel");
        }

        exportBtn.addActionListener(e -> EmployeeExcelHelper.handleEmployeeExport(table, this));
        importBtn.addActionListener(e -> EmployeeExcelHelper.handleEmployeeImport(this, () -> {
            loadMasterDataFromDb();
            refreshTableFromMaster(currentKeyword, currentDeptFilter);
        }));
    ...
    /** Sinh mã NV kế tiếp (VD: NV16 → NV17), dựa trên tất cả mã dạng NV+số trong CSDL. */
    private String generateNextMaNv() {
        int max = 0;
        Pattern pat = Pattern.compile("(?i)^NV(\\d+)$");
        for (NhanVienDTO nv : nhanVienHRDAO.getAll()) {
            if (nv.getManv() == null) {
                continue;
            }
            Matcher m = pat.matcher(nv.getManv().trim());
            if (m.matches()) {
    ...
    private static void focusComponent(JComponent c) {
    ...
    private void openAddEmployeeForm() {
        ...
        JTextField idField = new JTextField(generateNextMaNv(), 20);
        ...
        JDialog dialog = new JDialog(owner instanceof Frame ? (Frame) owner : null, "Thêm nhân viên mới", true);
        ...
        btnOk.addActionListener(e -> {
            ...
            // —— Kiểm tra theo thứ tự trên form; lỗi → báo + focus vào ô tương ứng ——
            if (manv.isEmpty()) {
                JOptionPane.showMessageDialog(dialog, "Mã NV không được trống!", "Lỗi nhập liệu", JOptionPane.WARNING_MESSAGE);
                focusComponent(idField);
                return;
            }
            if (nhanVienHRDAO.findById(manv) != null) {
                JOptionPane.showMessageDialog(dialog, "Mã NV đã tồn tại (không trùng)!", "Lỗi nhập liệu", JOptionPane.WARNING_MESSAGE);
                focusComponent(idField);
                return;
            }
            if (hoten.isEmpty()) {
            ...
            if (!ngayStr.isEmpty()) {
                try {
                    ...
                    JOptionPane.showMessageDialog(dialog, "Ngày vào làm không hợp lệ. Dùng định dạng yyyy-MM-dd.", "Lỗi nhập liệu", JOptionPane.WARNING_MESSAGE);
                    focusComponent(ngayVaoLamField);
                    return;
                }
            }
            ...
            try {
                ...
                focusComponent(soNgayPhepField);
                return;
            }
            ...
            if (nhanVienHRDAO.insert(dto)) {
                ...
                JOptionPane.showMessageDialog(dialog, "Lỗi khi lưu vào database.", "Lỗi", JOptionPane.ERROR_MESSAGE);
                focusComponent(idField);
            }
        });
    ...
    private void openEditEmployeeForm(int masterIndex) {
        ...
        Window owner = SwingUtilities.getWindowAncestor(this);
        JDialog dialog = new JDialog(owner instanceof Frame ? (Frame) owner : null, "Sửa nhân viên", true);
        dialog.setContentPane(root);
        dialog.getRootPane().setDefaultButton(btnOk);
        dialog.pack();
        dialog.setLocationRelativeTo(this);
        ...
        btnOk.addActionListener(e -> {
            ...
            String sDept = deptSel.toString();
            String maphongban = sDept.contains(" - ") ? sDept.substring(0, sDept.indexOf(" - ")).trim() : sDept;
