...
public class DepartmentManagementPanel extends JPanel {
    ...
    private JPanel createSearchSection() {
        ...
        exportBtn.setBorder(BorderFactory.createEmptyBorder(10, 16, 10, 16));
        exportBtn.setPreferredSize(new Dimension(108, 42));
        ...
        importBtn.setBorder(BorderFactory.createEmptyBorder(10, 16, 10, 16));
        importBtn.setPreferredSize(new Dimension(108, 42));
        ...
        if (currentUser == null || !permissionService.canExport(currentUser, "CN07")) {
            exportBtn.setEnabled(false);
            exportBtn.setToolTipText("Bạn không có quyền xuất Excel");
        }
        if (currentUser == null || !permissionService.canAdd(currentUser, "CN07")) {
            importBtn.setEnabled(false);
            importBtn.setToolTipText("Bạn không có quyền nhập Excel");
        }

        exportBtn.addActionListener(e -> DepartmentExcelHelper.handleDepartmentExport(this));
        importBtn.addActionListener(e -> DepartmentExcelHelper.handleDepartmentImport(this, this::refreshPanel));

        addBtn.addActionListener(e -> openAddDepartmentForm());

        wrapper.add(exportBtn);
        wrapper.add(Box.createHorizontalStrut(8));
        wrapper.add(importBtn);
        wrapper.add(Box.createHorizontalStrut(8));
        wrapper.add(addBtn);
        return wrapper;
    }
    ...
    /**
     * Sinh mã phòng ban kế tiếp (VD: PB04 → PB05), dựa trên các mã dạng PB+số trong CSDL.
     */
    private String generateNextMaPhongBan() {
        int max = 0;
        Pattern pat = Pattern.compile("(?i)^PB(\\d+)$");
        for (DepartmentDTO d : departmentService.getAllDepartments()) {
            if (d.getMaPhongBan() == null) {
                continue;
            }
            Matcher m = pat.matcher(d.getMaPhongBan().trim());
            if (m.matches()) {
    ...
    private void openAddDepartmentForm() {
        ...
        JTextField codeField = new JTextField(generateNextMaPhongBan(), 12);
